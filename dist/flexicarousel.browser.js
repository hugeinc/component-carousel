...
var Carousel = function () {
  ...
  */
			...
			if (!this.options.disableDragging) {
				if (this.isTouch) {
					['touchstart', 'touchmove', 'touchend', 'touchcancel'].map(function (event) {
						_this.handle.addEventListener(event, _this._bindings[event]);
					});
				} else {
   ...
   */
		...
		key: '_createBindings',
		value: function _createBindings() {
			return {
				// handle
		...
		value: function _slide(offset, animate) {
			...
			if (animate) {
				...
				setTimeout(function () {
					...
					_this2._removeClass(_this2.slideWrap, 'animate');
				}, delay);
			}
...
//# sourceMappingURL=flexicarousel.browser.js.map
